...
PERSPECTIVE_DISPLAY_LABELS = {
    "Customer is buyer | customer paper (1PP)": "𝐂𝐮𝐬𝐭𝐨𝐦𝐞𝐫 is 𝐛𝐮𝐲𝐞𝐫 | 𝐜𝐮𝐬𝐭𝐨𝐦𝐞𝐫 paper (1PP)",
    "Customer is buyer | counterparty paper (3PP)": "𝐂𝐮𝐬𝐭𝐨𝐦𝐞𝐫 is 𝐛𝐮𝐲𝐞𝐫 | 𝐜𝐨𝐮𝐧𝐭𝐞𝐫𝐩𝐚𝐫𝐭𝐲 paper (3PP)",
    "Customer is seller | customer paper (1PP)": "𝐂𝐮𝐬𝐭𝐨𝐦𝐞𝐫 is 𝐬𝐞𝐥𝐥𝐞𝐫 | 𝐜𝐮𝐬𝐭𝐨𝐦𝐞𝐫 paper (1PP)",
    "Customer is seller | counterparty paper (3PP)": "𝐂𝐮𝐬𝐭𝐨𝐦𝐞𝐫 is 𝐬𝐞𝐥𝐥𝐞𝐫 | 𝐜𝐨𝐮𝐧𝐭𝐞𝐫𝐩𝐚𝐫𝐭𝐲 paper (3PP)",
}
...
def _load_history_entry(entry: dict[str, Any]) -> None:
    ...
    st.session_state.agreement_perspective = _normalize_perspective(
        context.get("agreement_perspective", PERSPECTIVE_DEFAULT)
    )
...
def _format_perspective_option(perspective: str) -> str:
    return PERSPECTIVE_DISPLAY_LABELS.get(perspective, perspective)
...
def _context_form(template: dict[str, Any], rules: dict[str, Any], prompt_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if st.session_state.get("agreement_perspective", PERSPECTIVE_DEFAULT) not in PERSPECTIVE_OPTIONS:
        st.session_state.agreement_perspective = PERSPECTIVE_DEFAULT
    agreement_perspective = st.selectbox(
        "Perspective",
        PERSPECTIVE_OPTIONS,
        key="agreement_perspective",
        format_func=_format_perspective_option,
        help="Sets whether the agent analyzes from the customer's buyer/seller position and whether mock agreements should use customer paper (1PP) or counterparty paper (3PP).",
    )
...
def _perspective_instruction(context: dict[str, Any]) -> str:
    perspective = context.get("agreement_perspective", PERSPECTIVE_INFER) or PERSPECTIVE_INFER
    if perspective == PERSPECTIVE_INFER:
        return (
            "Infer buyer/seller perspective and paper type from the request and available agreement evidence. "
            "If unclear, state the assumption briefly before giving findings."
        )
    if "buyer" in perspective.lower() and "1pp" in perspective.lower():
        return (
    ...
    if "buyer" in perspective.lower() and "3pp" in perspective.lower():
        return (
    ...
    if "seller" in perspective.lower() and "1pp" in perspective.lower():
        return (
    ...
    if "seller" in perspective.lower() and "3pp" in perspective.lower():
        return (
...
- Selected perspective: {context.get('agreement_perspective', PERSPECTIVE_DEFAULT)}
...
- If perspective is inferred, choose the paper type that best supports the use case and state the assumption in the agreement pack summary.
