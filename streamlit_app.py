...
def _render_prompt_builder(
    ...
    if generation_mode == MODE_GEMINI:
        ...
        st.text_area("Copy-ready Agent Studio brief", value=generated_prompt, height=320, label_visibility="collapsed")
...
def _source_prompt_title(prompt: str) -> str:
    for line in prompt.splitlines():
        ...
        if not cleaned:
            continue
        if cleaned.lower() in {"ai agent prompt", "agent prompt", "agent studio brief"}:
            continue
