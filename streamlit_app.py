...
def _context_form(template: dict[str, Any], rules: dict[str, Any], prompt_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if generation_mode in {MODE_USE_LIBRARY, MODE_GEMINI}:
...
def _qa_wizard(
    ...
    if missing_fields:
        st.info(f"Complete {', '.join(missing_fields)} before validating an agent response.")
        return
    ...
    output_format = st.selectbox(
        ...
        ALLOWED_OUTPUT_FORMATS,
        index=_safe_index(ALLOWED_OUTPUT_FORMATS, default_output_format),
...
- Then include a separate section titled "Workflow Data Outputs".
- In "Workflow Data Outputs", return every configured Data Output using exact variable names and workflow-safe values.
- The workflow variables must be deterministic enough for routing decisions, for example `Risk_Level = High` when the agent finds 3 or more risky terms.
- Do not invent agreement terms, dates, commercial values, source evidence, or business facts.
    ...
    if _uses_workflow_destination(context) and _uses_chat_destination(context):
        return "\n".join(
            [
                ...
                "- Then include a separate Workflow Data Outputs section.",
                "- Use exact variable names and deterministic values in Workflow Data Outputs.",
                "- Do not wrap workflow variables in markdown fences unless JSON is explicitly requested.",
            ]
        )
    return "\n".join(
        [
    ...
    if _uses_workflow_destination(context) and _uses_chat_destination(context):
        return "\n".join(
            [
                ...
                "- Then include a separate Workflow Data Outputs section with exact variable names and workflow-safe values.",
                "- Do not show underscores in the customer-facing chat brief.",
                "- Keep workflow variable names exact in the Workflow Data Outputs section.",
            ]
        )
