...
def _render_knowledge_sources_section(context: dict[str, Any]) -> list[dict[str, str]]:
    _render_anchor("knowledge-sources")
    _render_form_section_label("Knowledge Sources", size="primary")
    st.markdown(
        """
        <div class="ds-library-note">
            Required source files appear below based on the agent objective.
        </div>
        """,
    ...
    if missing_source_fields:
        st.info("Complete the Agent Objective fields to generate required Knowledge Sources.")
        return []

    required_sources = _required_knowledge_sources(context)
    _sync_knowledge_source_selection_state(context, required_sources)
    selected_sources = _render_knowledge_source_selector(context, required_sources)
    if not selected_sources:
        st.warning("Select at least one Knowledge Source or generate a custom knowledge source.")
...
def _sync_knowledge_source_selection_state(context: dict[str, Any], sources: list[dict[str, str]]) -> None:
    ...
    for key in list(st.session_state.keys()):
        if key.startswith("ks_include_recommended_") or key in {
...
def _render_knowledge_source_selector(
    context: dict[str, Any],
    ...
    for source in required_sources:
        ...
        if len(required_sources) > 1:
            status_pill = '<span class="ds-status-pill">Required</span>'
        st.markdown(
            f"""
            <div class="ds-ks-required-item">
                <div>
                    <strong>{escape(source['name'])}</strong>
                    <p>{escape(source['purpose'])}</p>
                </div>
                {status_pill}
            </div>
            """,
    ...
    if include_custom_source:
        custom_source_value = st.text_area(
        ...
        context["custom_knowledge_source_description"] = ""
        context["custom_knowledge_sources"] = []
...
def _render_prompt_mode_selector() -> str:
    ...
    cards = [
        (MODE_USE_LIBRARY, "Use Prompt Library", "Start with a proven SC demo prompt."),
        (MODE_CUSTOMIZE, "Customize Existing Prompt", "Adapt a prompt that already works."),
...
def _prompt_library_form(default_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    st.markdown(
        """
        <div class="ds-library-note">
            Choose the closest starting prompt from the library. Filter by business area or search by title.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    selected_label = st.selectbox("Starting Prompt", titles, index=title_index, key="library_title")
...
def _context_form(template: dict[str, Any], rules: dict[str, Any], prompt_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    _render_form_section_label("Agent Objective & Inputs")
    ...
    _render_form_section_label("Perspective", size="secondary")
    ...
    context["knowledge_sources"] = _render_knowledge_sources_section(context)
