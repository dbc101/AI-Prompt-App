...
def _render_output_rules(rules: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    elif context["output_destination"] in {"Workflow", "Salesforce"}:
...
def _response_word_limit(context: dict[str, Any]) -> int | None:
    destination = context.get("output_destination", "")
    if destination in {"Workflow", "Salesforce"}:
        return None
...
def _rules_for_validation(rules: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    limits = dict(rules.get("customer_output_limits", {}))
...
def _generic_quality_report(sample_output: str, context: dict[str, Any], generated_prompt: str) -> dict[str, Any]:
    ...
    max_words = _response_word_limit(context)
    if max_words and word_count > max_words:
        issues.append(f"The sample output is too long for a live demo ({word_count} words). Keep it under {max_words} words.")
    ...
    if text and (not max_words or word_count <= max_words):
    ...
    status = "PASS" if text and not issues else "FAIL"
...
def _render_generic_report(report: dict[str, Any]) -> None:
    ...
    _render_prompt_copy_action(report["optimized_prompt"], disabled=False, button_label="Copy Tune-Up Prompt", key_suffix="generic_tune_up")
...
def _render_report(report: dict[str, Any]) -> None:
    ...
    _render_prompt_copy_action(optimized_prompt, disabled=False, button_label="Copy Tune-Up Prompt", key_suffix="renewal_tune_up")
