...
CUSTOMER_ROLE_DISPLAY_LABELS = {
    CUSTOMER_ROLE_BUYER: "𝐁𝐮𝐲𝐞𝐫",
    CUSTOMER_ROLE_SELLER: "𝐒𝐞𝐥𝐥𝐞𝐫",
}
...
PAPER_TYPE_DISPLAY_LABELS = {
    PAPER_CUSTOMER: "𝐂𝐮𝐬𝐭𝐨𝐦𝐞𝐫 paper (1PP)",
    PAPER_COUNTERPARTY: "𝐂𝐨𝐮𝐧𝐭𝐞𝐫𝐩𝐚𝐫𝐭𝐲 paper (3PP)",
    PAPER_MIXED: "𝐌𝐢𝐱𝐞𝐝 1PP + 3PP",
}
...
def _render_prompt_builder(
    ...
    _render_section_intro(
        "Output Generation",
        "Generate a copy-ready AI Chat prompt, Agent Studio prompt, or draft Agent Studio description from the intake details above.",
    )
    generation_mode = _normalize_generation_mode(context["generation_mode"])
    st.caption("The output updates automatically as the intake details are completed.")
    ...
    if generation_mode == MODE_GEMINI:
        ...
        st.text_area("Draft Agent Description", value=generated_prompt, height=320, label_visibility="collapsed")
        _render_agent_studio_actions(generated_prompt, disabled=bool(missing_fields))
    else:
        st.subheader("Generated Prompt")
        st.text_area("Copy-ready optimized prompt", value=generated_prompt, height=280, label_visibility="collapsed")
...
def _context_form(template: dict[str, Any], rules: dict[str, Any], prompt_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    with right:
        contract_type = st.text_input(
            ...
            value="",
            key="contract_type",
            help="The main agreement category the demo is centered on, such as Supplier Agreement, MSA, Order Form, Insertion Order, or NDA.",
        )
    ...
    _render_form_section_label("Perspective")
    ...
    document_scope = st.text_area(
        "Source Documents / Data to Review",
        value="",
        height=80,
        key="document_scope",
        placeholder="Example: Supplier MSAs, order forms, amendments, renewal notices, spend metadata, and contract owner fields.",
        help="The full set of documents, metadata, or business records the agent should use as source evidence.",
    )
...
def _missing_prompt_fields(context: dict[str, Any]) -> list[str]:
    required_labels = {
        "audience": "LOB",
        "contract_type": "Primary Agreement Type",
        "industry": "Industry",
        "use_case": "Customer Use Case",
        "document_scope": "Source Documents / Data to Review",
...
- Customer use case: {context.get('use_case', 'Not specified')}
- Primary agreement type: {context.get('contract_type', 'Not specified')}
...
- Primary Agreement Type: {context['contract_type']}
...
- Name files using the invented demo company name, agreement type, and a short document role, for example `Demo_Customer_{context['contract_type']}_MSA.pdf`, `Demo_Customer_Order_Form.pdf`, or `Demo_Customer_Renewal_Notice.pdf`.
...
- If this supplement conflicts with required structured fields, schema-safe values, Data Outputs, or the Workflow Prompt, preserve the structured requirements and apply the supplement only to the user-facing explanation.
...
{response_contract}"""
    ...
    action_rules = _agent_studio_action_rules_text(context, rules)
    agent_studio_source_rule = (
        "- Use configured tool outputs, workflow filters, Data Outputs, and decision logic as the source of truth for repeatable workflow runs."
        if _uses_workflow_destination(context)
        else "- Use configured tool outputs and available in-scope records as the source of truth; do not add workflow variables, Data Outputs, or a Workflow Prompt unless workflow configuration is enabled."
    )
...
- This prompt is intended for a configured, repeatable Agent Studio agent, not ad-hoc Iris chat.
...
- Treat this prompt's role, objective, demo context, perspective, data scope, destination, response contract, evidence rules, missing-data rules, and next-action rules as hard requirements.
...
- Stay within the described demo, data, perspective, and agreement scope.
...
def _optimized_response_contract(context: dict[str, Any], rules: dict[str, Any]) -> str:
    ...
    if _uses_agent_studio_destination(context):
        workflow_instruction = (
            "- Keep workflow variables, Data Outputs, and the Workflow Prompt separate from the user-facing answer unless Agent Studio explicitly requests structured output."
            if _uses_workflow_destination(context)
            else "- Do not add workflow variables, Data Outputs, or a Workflow Prompt to the prompt unless workflow configuration is enabled."
        )
