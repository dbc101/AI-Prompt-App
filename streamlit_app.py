...
def _render_output_rules(rules: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    st.markdown("- Make every visible title and section subheader bold in the agent's response.")
...
def _evidence_instruction() -> str:
    return "Include 1-3 short evidence quotes or source references when needed to support multiple findings."
...
def _table_cell_instruction() -> str:
    return "If using a table, aim for table cells under 12 words; do not exceed 18 words unless clause language needs the extra context."
...
def _dynamic_response_structure_text(context: dict[str, Any]) -> str:
    ...
    example_heading = profile["sections"][0].split(" - ", 1)[0]
...
- Use markdown bold for every visible title and subheader, for example `{example_heading}` or `**Recommended action**`.
