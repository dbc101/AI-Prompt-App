...
def _render_agent_studio_configuration(context: dict[str, Any]) -> None:
    ...
    include_workflow_config = st.checkbox(
        ...
        help="Turn this on only when the agent prompt needs structured Data Outputs or workflow routing logic.",
    )
    if not include_workflow_config:
        st.caption("Workflow Data Outputs and routing logic are not added to the generated prompt unless this is enabled.")
        return
...
Do not include Data Outputs, workflow variables, or routing logic unless the SC explicitly enables workflow configuration."""
...
{response_contract}"""
    ...
    agent_studio_source_rule = (
        ...
        if _uses_workflow_destination(context)
        else "- Use configured tool outputs and available in-scope records as the source of truth; do not add workflow variables, Data Outputs, or routing logic unless workflow configuration is enabled."
    )
...
def _optimized_response_contract(context: dict[str, Any], rules: dict[str, Any]) -> str:
    ...
    if _uses_agent_studio_destination(context):
        workflow_instruction = (
            "- Keep workflow variables, Data Outputs, and routing logic separate from the user-facing answer unless Agent Studio explicitly requests structured output."
            if _uses_workflow_destination(context)
            else "- Do not add workflow variables, Data Outputs, or routing logic to the prompt unless workflow configuration is enabled."
        )
...
def _library_output_style_instructions(context: dict[str, Any]) -> str:
    ...
    if _uses_agent_studio_destination(context):
        ...
        if _uses_workflow_destination(context):
            instructions.append("- Treat Data Outputs and routing logic as Agent Studio configuration, not extra prose for the final user-facing answer.")
        else:
            instructions.append("- Do not add Data Outputs, workflow variables, or routing logic unless workflow configuration is enabled.")
        return "\n".join(instructions)
    if _uses_workflow_destination(context) and not _uses_chat_destination(context):
        return "\n".join(
            [
...
def _output_style_instructions(context: dict[str, Any]) -> str:
    ...
    if _uses_agent_studio_destination(context):
        ...
        if _uses_workflow_destination(context):
            ...
            instructions.append("- Do not include Data Outputs, workflow variables, or routing logic in the prompt or user-facing answer.")
        return "\n".join(instructions)
