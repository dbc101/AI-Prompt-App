...
def _render_prompt_builder(
    ...
    action_label = "Generate Gemini Brief" if generation_mode == MODE_GEMINI else "Generate Optimized Prompt"
    ...
    if generation_mode == MODE_GEMINI:
        ...
        st.text_area("Copy-ready optimized prompt", value=generated_prompt, height=560, label_visibility="collapsed")
        ...
        <div class="ds-workflow" aria-label="Demo prep workflow">
            ...
            <a class="ds-workflow-step" href="#customize-prompt">
                ...
                <div>
                    ...
                    <small>Customer and demo context</small>
                </div>
            </a>
            <a class="ds-workflow-step" href="#prompt-builder">
                ...
                <div>
                    ...
                    <small>Copy optimized output</small>
                </div>
            </a>
            <a class="ds-workflow-step" href="#agreement-builder">
                <span>4</span>
                <div>
                    <strong>Build Agreements</strong>
                    <small>Create mock PDFs</small>
                </div>
            </a>
            <a class="ds-workflow-step" href="#prompt-validation">
                ...
                <div>
                    ...
                    <small>Check agent accuracy</small>
                </div>
            </a>
        </div>
        """,
...
def _build_generated_artifact(context: dict[str, Any], template: dict[str, Any], rules: dict[str, Any]) -> str:
    ...
    if generation_mode == MODE_CUSTOMIZE:
        base_prompt = _build_customized_prompt(context, template, rules)
    else:
        base_prompt = build_prompt(context, template, rules)
    return _build_initial_optimized_prompt(base_prompt, context)


def _build_initial_optimized_prompt(base_prompt: str, context: dict[str, Any]) -> str:
    """Apply the same deterministic quality bar before the SC ever tests the agent."""
    return build_optimized_prompt(
        base_prompt,
        _upfront_prompt_recommendations(context),
        context["output_destination"],
    )


def _upfront_prompt_recommendations(context: dict[str, Any]) -> list[str]:
    recommendations = [
        "State the agent objective, data scope, and audience in plain language.",
        "Require the response to be concise, deterministic, and easy to speak to in a live demo.",
        "Require source evidence for material findings and prohibit unsupported assumptions.",
        "Define how the agent should handle missing information before it answers.",
        "Include one clear next action, owner, or recommendation when the use case calls for action.",
    ]
    if context["output_destination"] in {"Workflow", "Salesforce"}:
        recommendations.extend(
            [
                "Use strict field names only for system-facing outputs.",
                "Apply the selected Data Outputs and Decision Logic exactly.",
            ]
        )
    else:
        recommendations.append("Use readable customer-facing labels and avoid underscore-style field names.")
...
def _render_prompt_copy_action(prompt: str, disabled: bool) -> None:
    if disabled:
        st.button("Copy Prompt", disabled=True, key="disabled_copy_prompt")
        return
    ...
    components.html(
        ...
        <div class="gemini-actions">
            <button id="copy-optimized-prompt" type="button">Copy Prompt</button>
            <span id="copy-prompt-status" aria-live="polite"></span>
        </div>
        <script>
            const optimizedPrompt = {escaped_prompt};
            const copyPromptButton = document.getElementById("copy-optimized-prompt");
...
def _render_gemini_actions(brief: str, disabled: bool) -> None:
    if disabled:
        left, right = st.columns(2)
        left.button("Copy Gemini Brief", disabled=True, key="disabled_copy_gemini_brief")
...
def _render_prompt_readiness(context: dict[str, Any], generated_prompt: str, missing_fields: list[str]) -> None:
    ...
    title = "Gemini Brief Readiness" if _normalize_generation_mode(context.get("generation_mode", "")) == MODE_GEMINI else "Optimized Prompt Readiness"
...
def _qa_wizard(template_id: str, context: dict[str, Any], generated_prompt: str, rules: dict[str, Any]) -> None:
    ...
    sample_output = st.text_area("Paste sample AI Agent output", value="", height=340, key="renewal_sample_output")

    if st.button("Validate Output", type="primary", key="renewal_validate_output"):
...
def _render_generic_report(report: dict[str, Any]) -> None:
    ...
    st.text_area("Prompt tune-up", value=report["optimized_prompt"], height=420, label_visibility="collapsed")
