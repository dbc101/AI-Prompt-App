...
def main() -> None:
    st.set_page_config(page_title="Agent & Worksheet Prompt Builder", layout="wide")
    _inject_styles()
    ...
    prompt_tab, qa_tab, agreement_pack_tab = st.tabs(["Prompt Builder", "QA Wizard", "Agreement Pack Builder"])
...
def _render_prompt_builder(
    ...
    _render_section_intro(
        "Output Generation",
        "Generate the copy-ready prompt, customized prompt, or Gemini brief from the intake details above.",
    )
...
def _render_agreement_pack_builder(
    ...
    _render_section_intro(
        "Agreement Pack",
        "Create a Gemini-ready brief for fictional agreements and metadata that support the selected demo story.",
    )
    st.button("Generate Agreement Pack Brief", type="primary", disabled=bool(missing_fields))
    st.subheader("Generated Agreement Pack Brief")
    _render_agreement_pack_steps()
    if missing_fields:
        st.info(f"Complete {', '.join(missing_fields)} to generate the Agreement Pack Brief.")
    st.text_area("Copy-ready Agreement Pack Brief", value=agreement_pack_brief, height=560, label_visibility="collapsed")
    _render_agreement_pack_actions(agreement_pack_brief, disabled=bool(missing_fields))
    st.download_button(
        "Download Agreement Pack Brief",
...
def _render_app_header() -> None:
    st.markdown(
        ...
        <div class="ds-hero">
            ...
            <h1>Agent &amp; Worksheet Prompt Builder</h1>
            <p>Generate and Optimize Agent and Worksheet Prompts</p>
        </div>
        """,
    ...
    st.markdown(
        ...
        <div class="ds-workflow" aria-label="Demo prep workflow">
            <div class="ds-workflow-step">
                <span>1</span>
                <strong>Choose Prompt Mode</strong>
            </div>
            <div class="ds-workflow-step">
                <span>2</span>
                <strong>Enter Demo Details</strong>
            </div>
            <div class="ds-workflow-step">
                <span>3</span>
            ...
            <div class="ds-workflow-step">
                <span>5</span>
                <strong>Build Source Data</strong>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def _render_generation_mode_helper(generation_mode: str) -> None:
    mode_guidance = {
        MODE_USE_LIBRARY: "Start from a Pre-built AI Agent or Worksheet prompt.",
...
def _prompt_library_form(default_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    st.markdown(
        ...
        <div class="ds-library-note">
            Pick a type, category, then the closest agent or worksheet.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _context_form(template: dict[str, Any], rules: dict[str, Any], prompt_library: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    _render_form_section_label("Agent / Worksheet Objective")
...
def _render_gemini_mode_steps() -> None:
    st.markdown(
        ...
        <div class="ds-steps ds-steps-compact">
            Copy the brief into Gemini, then validate the AI Agent response in the QA Wizard.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _render_agreement_pack_actions(brief: str, disabled: bool) -> None:
    if disabled:
        left, right = st.columns(2)
        left.button("Copy Agreement Pack Brief", disabled=True)
        right.button("Paste in Gemini", disabled=True)
        return
    ...
    components.html(
        f"""
        <div class="gemini-actions">
            <button id="copy-agreement-pack-brief" type="button">Copy Agreement Pack Brief</button>
...
def _render_prompt_readiness(context: dict[str, Any], generated_prompt: str, missing_fields: list[str]) -> None:
    ...
    st.markdown(
        ...
        <div class="ds-readiness-panel">
            ...
            {issue_block}
            <div class="ds-readiness-note">This does not replace QA Wizard testing; it confirms the prompt has the core ingredients before you run it.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _prompt_readiness_report(context: dict[str, Any], generated_prompt: str, missing_fields: list[str]) -> dict[str, Any]:
    ...
    if generation_mode == MODE_GEMINI:
        ...
        checks.extend(
            [
                ("Role and task", _contains_any(text, ["agent role", "worksheet prompt", "your job", "create an ai-generated worksheet"]), "Add a clear role or task."),
                ("Objective", bool(context.get("agent_objective", "").strip()) and _contains_any(text, ["agent objective", "worksheet objective"]), "Add the agent or worksheet objective."),
...
def _qa_wizard(template_id: str, context: dict[str, Any], generated_prompt: str, rules: dict[str, Any]) -> None:
    ...
    _render_section_intro(
        "QA Validation",
        "Paste the AI Agent or Worksheet output from your demo environment and the app will flag formatting, quality, and demo-readiness issues.",
    )
...
def _load_prompt_library_from_csv_rows(rows: list[list[str]]) -> list[dict[str, Any]]:
    ...
    if any("worksheet" in header for header in headers):
        return _worksheet_entries(rows)
