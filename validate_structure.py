...
def _validate_object(output: dict[str, Any], rules: dict[str, Any], result: StructuralResult) -> None:
    ...
    for field_name in required_fields:
        ...
        if _is_blank(output[field_name]) and not _field_can_be_empty(field_name, output, rules):
            _add_issue(result, "error", field_name, f"`{field_name}` is blank.", "Return a value or the destination-safe missing value allowed by the schema.", "blank_required_field")
        else:
        ...
        for child_field in child_fields:
            ...
            if _is_blank(child_value):
                _add_issue(
                    ...
                    "Return every required child field or use the destination-safe missing value allowed by the schema.",
    ...
    requires_recommended_action = (
        "recommended_action" in required_fields
        or "recommended_action" in output
        or bool(rules.get("recommended_action_required_fields"))
    )
    if requires_recommended_action:
...
def _validate_unfavorable_term(term: Any, index: int, rules: dict[str, Any], result: StructuralResult) -> None:
    ...
    for field_name in rules.get("unfavorable_term_required_fields", []):
...
def _check_hallucination_signals(output: dict[str, Any], rules: dict[str, Any], result: StructuralResult) -> None:
    missing_phrase_value = rules.get("missing_information_phrase", "Not found in agreement")
    missing_phrase = missing_phrase_value.lower()
        ...
        for term in watch_terms:
            if term in full_text:
                _add_issue(
                    ...
                    f"Use extracted agreement language or `{missing_phrase_value}` instead of implying source terms.",
                    "hallucination_risk",
                )
                break
        return
    ...
    for term in watch_terms:
        if term in non_schema_text and not any(term in evidenced_term for evidenced_term in evidenced_terms):
            _add_issue(
                ...
                f"Add an unfavorable term with extracted language, or use `{missing_phrase_value}` when the source is missing.",
                "hallucination_risk",
            )
            break
