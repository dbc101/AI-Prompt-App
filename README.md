# DocuSign IAM Agent Configuration Builder

A lightweight internal Streamlit app for DocuSign Solution Consultants who need to create, check, and optimize DocuSign IAM-style AI Agent prompts for demos, POCs, workflow design, and customer-specific solution storytelling.

The MVP includes a bundled AI Agent prompt library from `data/agent_worksheet_prompts.xlsx`, plus demo examples. Demo-Readiness Check now works across the library: it derives field-level checks from the selected prompt pattern, Data Outputs, sample output format, and destination, while renewal-intelligence prompts still use the deepest template-specific rule path.
...
- Checks prompt outputs across library patterns using dynamic required fields, selected output format, destination-specific formatting rules, built-in expected-output fallbacks, and demo-readiness checks.
- Preserves deeper renewal-intelligence checks for required fields, allowed values, renewal risk details, commercial summary fields, and recommended action details.
...
- Applies audience-aware length limits: roughly 175 words for leadership/executive summaries, roughly 225 words for standard AI Chat or Agent Studio demos, no prose word limit for Workflow-only structured output, and roughly 300 words for internal analysis/debugging.
...
The app does not require secrets for the current template-based, prompt-library, Agent Studio brief, QA, or agreement-pack workflows.
...
3. Complete the **Prompt Intake Form**: customer, LOB, agreement type, industry, customer use case, documents/data reviewed, and objective.
4. Click **Generate Optimized Prompt** or **Generate Agent Studio Brief**.
5. Copy the optimized prompt or Agent Studio Brief and paste it into Agent Studio.
6. Use **Agreement Builder** when it appears for agreement-heavy patterns that need a Gemini-ready brief for downloadable mock signed PDF agreements.
7. Upload the generated mock agreement PDFs into the demo environment, then run the AI Agent prompt.
8. Paste the AI Agent output into **Demo-Readiness Check** and review the result, issue list, quality scores, prompt improvements, optional prompt tune-up, and retest checklist.

Use **Prompt History** in the sidebar to start a new prompt, save the current generated prompt or Agent Studio brief, reload recent prompts from the current session, or clear the session history. History is local to the active app session and is not a long-term storage system.
...
**Customize Existing Prompt** lets SCs paste a prompt that already works well, then adapts and optimizes it for the selected customer, industry, LOB, customer use case, documents/data, and objective.

**Create New** skips the Prompt Library and creates a copy-ready Agent Studio brief from the intake context. SCs copy the brief, open Agent Studio, paste it into the AI-generated agent prompt builder, then run the generated prompt and check the sample output in Demo-Readiness Check.

The app defaults generated artifacts to Agent Studio so SCs do not need to choose a prompt destination. When workflow configuration is enabled, the form shows generated **Data Outputs** and **Workflow Routing Logic** because those configuration options are embedded in the Agent Studio experience.
...
Use the included passing and failing samples to see the readiness-check behavior quickly.
...
Set `status: active` only after rules and readiness-check behavior are ready. Stubbed templates can stay in the file with `status: stub`.

## How To Add New Readiness Rules

Demo-Readiness Check can run without a custom rules file by deriving required fields from the selected prompt pattern's Data Outputs. Create or extend a YAML file in `rules/` when a prompt pattern needs deeper domain-specific checks. The renewal intelligence MVP uses `rules/completed_agreements_renewal_intelligence_rules.yaml`, which defines:
