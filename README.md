...
3. Complete the **Prompt Intake Form**: customer, LOB, agreement type, industry, customer use case, documents/data reviewed, objective, and prompt destination.
4. Click **Generate Optimized Prompt** or **Generate Agent Studio Brief**.
5. Copy the optimized prompt into AI Chat, or copy the Agent Studio Brief and paste it into Agent Studio.
...
When **Agent Studio** is selected, the form shows generated **Data Outputs** and **Workflow Routing Logic** because those configuration options are embedded in the Agent Studio experience.

In **Prompt Destination**, choose one option: **AI Chat** for ad-hoc Iris prompts, or **Agent Studio** when publishing the prompt as an IAM Agent. Agent Studio prompts include an operating contract so platform safety/configured tools stay first while the generated prompt remains the behavior contract.
