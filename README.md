# DocuSign IAM AI Agent Prompt Builder + QA Wizard
...
The MVP includes a bundled prompt library from `data/agent_worksheet_prompts.xlsx`, including AI Agent prompts, Worksheet prompts, use-case ideas, and demo examples. Renewal-intelligence prompts use the deepest field-by-field QA path; other library prompts use a general demo-readiness QA check.

## What The App Does

- Lets SCs select an AI Agent or Worksheet pattern from the Prompt Library.
- Generates a customer-specific prompt from the selected library pattern and SC-entered demo context.
- Applies the expected output rules and allowed values behind the scenes.
- Accepts pasted or uploaded sample AI Agent output in JSON, markdown table, HTML table, workflow variables, or plain language.
...
- Returns PASS, NEEDS OPTIMIZATION, or FAIL with issue details, recommended prompt improvements, an optimized prompt, and a retest checklist.
...
When the correct app is running, the page shows the Docusign IAM sidebar, the colorful branded banner, blank Agent Intake Form fields, and an empty generated prompt area until required fields are completed.

## Publish On Streamlit Community Cloud

Use `streamlit_app.py` as the main file path. Keep the full project structure together so the bundled workbook, YAML files, validators, samples, and branded stylesheet are available at runtime.
...
2. Complete the **Agent Intake Form**: customer, LOB, agreement type, agent destination, industry, customer/evaluation use case, documents/data reviewed, and objective.
3. If using **Use Prompt Library**, choose the prompt type, category, and agent or worksheet name from the **Prompt Library**.
4. Copy the generated prompt or Gemini Agent Brief into the right workspace.
5. Paste or upload the AI Agent output into the QA Wizard.
6. Review the validation result, issue list, quality scores, prompt improvements, optimized prompt, and retest checklist.
7. Use **Agreement Pack Builder** to create a Gemini-ready brief for fictional demo agreements when the demo environment needs better source data.
...
**Use Prompt Library** adapts the selected AI Agent or Worksheet pattern from the bundled workbook.

**Customize Existing Prompt** lets SCs paste a prompt that already works well, then adapts it to the selected customer, industry, LOB, customer/evaluation use case, destination, documents/data, and objective.

**Generate Gemini Brief** creates a copy-ready Gemini Brief. The app does not call Gemini directly or store Gemini credentials. SCs copy the brief, open Gemini, paste it into their approved enterprise Gemini workspace, then run the generated prompt and validate the sample output in the QA Wizard.
...
The **Prompt Library** reads AI Agent and Worksheet patterns from `data/agent_worksheet_prompts.xlsx`.

When **Use Prompt Library** is selected, SCs can select:

- Prompt type: AI Agent or Worksheet
- Category
- Agent or worksheet name

The selected library pattern becomes the starting point for the generated prompt. The **Customer / evaluation use case** field is intentionally separate; it captures the broader customer scenario, while the selected library pattern captures the specific agent or worksheet behavior.

The library is bundled with the app so SCs do not need to manage or upload a separate spreadsheet.

## Agreement Pack Builder

The **Agreement Pack Builder** creates a copy-ready brief for Gemini or another approved document-generation workspace. It asks Gemini to create fictional, demo-only executed agreements and supporting metadata that align to the selected prompt-library pattern, industry, LOB, customer/evaluation use case, agreement type, source documents, and agent objective.

Use this when the demo environment needs stronger source data before running the AI Agent prompt. The generated source documents should be marked fictional and should never contain real customer data.
