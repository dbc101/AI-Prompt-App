...
6. Use **Agreement Builder** to create a Gemini-ready brief for downloadable fictional signed PDF agreements when the demo environment needs better source data.
7. Upload the generated mock agreement PDFs into the demo environment, then run the AI Agent prompt.
8. Paste the AI Agent output into **Prompt Validation** and review the validation result, issue list, quality scores, prompt improvements, optimized prompt, and retest checklist.
...
**Use Prompt Library** adapts the selected AI Agent pattern from the bundled workbook.
...
The **Agreement Builder** creates a copy-ready brief for Gemini or another approved document-generation workspace. It asks Gemini to create downloadable fictional, demo-only signed PDFs and supporting metadata that align to the selected prompt-library pattern, industry, LOB, customer/evaluation use case, agreement type, source documents, and agent objective.

Use this when the demo environment needs stronger source data before running the AI Agent prompt. The generated source documents should be marked fictional, should use mock typed signatures only, and should never contain real customer data.
